...
namespace JANOARG.Chartmaker.UI.Modal.ModalTypes
{
    public class HomeModal : Modal
    {
        ...
        public new void Start() 
        {
            ...
            List<RecentSong> list = new(Behaviors.Chartmaker.Chartmaker.main.RecentSongsStorage.Get("List", new RecentSong[] {}));
            foreach (RecentSong recent in list) 
            {
                ...
                item.Button.onClick.AddListener(() => {
                    Behaviors.Chartmaker.Chartmaker.main.LoaderPanel.SetSong(recent.SongName, recent.SongArtist, recent.BackgroundColor, recent.InterfaceColor);
                    StartCoroutine(Behaviors.Chartmaker.Chartmaker.main.OpenSongRoutine(recent.Path));
                });
        ...
        private void ShowRecentSongRightClickMenu(RecentSongItem item, RecentSong recent)
        {
            ContextMenuHolder.main.OpenRoot(new ContextMenuList(
                ...
                new ContextMenuListAction("Remove from List", () =>
                {
                    ...
                    Behaviors.Chartmaker.Chartmaker.main.RemoveFromRecent(index);
        ...
        private IEnumerator LoadIconImageRoutine(RecentSongItem item, string path)
        {
            ...
            {
                UnityEngine.Debug.LogWarning($"Icon for {item.SongArtistLabel.text} - {item.SongNameLabel.text} failed to load: ${activeTask.Exception}");
                yield break;
            }
        ...
        {
            Behaviors.Chartmaker.Chartmaker.main.OpenSongModal();
        }
